...
- ✅ **E9 + E6 — AI proxy + real review** — thin Express + Anthropic-SDK proxy (`server/`) holding the
  API key; `POST /review` returns a typed `Review` via **structured outputs** on **`claude-opus-5`**.
  Frontend `aiClient.reviewExercise()` calls the proxy when `VITE_API_BASE` is set and **falls back to
  the mock** on any failure (no proxy, no key, network/CORS error), signalled in the UI. Verified:
  proxy `/health` + validation + keyless 503; end-to-end call from the app (CORS preflight → POST →
  503 → graceful mock fallback). The successful-LLM path needs the user's key and was not exercised.
...
- **2026-08-08** — **E9 + E6 (AI proxy + real review) built.** Thin Express/Anthropic-SDK proxy in `server/` (`POST /review`, structured outputs, `claude-opus-5`); frontend calls it via `VITE_API_BASE` with graceful mock fallback. Verified proxy health/validation and end-to-end fallback in-browser (real LLM path needs the user's key). **The whole app now runs on real AI when a key is supplied — mock only as fallback.**
